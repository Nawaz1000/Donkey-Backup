...
def get_compressor_info(log_path: str = None) -> dict:
    threads = max(1, min(4, os.cpu_count() - 1)) if hasattr(os, "cpu_count") and os.cpu_count() else 1
...
def stream_backup_to_storage(cmd: list, env: dict, storage: dict, remote_name: str, log_path: str, compression_cmd: list = None, tracker: ProgressTracker = None) -> str:
    ...
    # Scale chunk size and queue capacity for higher throughput (16MB chunk, 16 capacity = 256MB buffer)
    ...
    reader_thread = threading.Thread(
        ...
        args=(stdout_stream, q, stop_event, 16 * 1024 * 1024, reader_tracker)  # 16MB chunks
    )
    ...
    try:
        if storage["type"] == "azure":
            ...
            client = BlobServiceClient.from_connection_string(conn_str, max_block_size=16 * 1024 * 1024, connection_timeout=300, read_timeout=3600)
            ...
            blob.chunk_size = 16 * 1024 * 1024  # 16MB chunk size
    ...
    with open(log_path, "a") as err_file:
        ...
        try:
            if storage["type"] == "azure":
                ...
                client = BlobServiceClient.from_connection_string(conn_str, max_single_get_size=16 * 1024 * 1024, max_chunk_get_size=16 * 1024 * 1024, connection_timeout=300, read_timeout=3600)
                container = client.get_container_client(storage["azure_container"])
                stream = container.download_blob(remote_name, max_concurrency=8, read_timeout=3600)
